...
import {
  ...
  getCurrentBalance,
  getInitialBalance,
} from "../../transaction/all/[pid]/[basketname]"
  ...
  try {
    ...
    if (email.length === 0) {
      ...
      return res.status(403).json({
        message: "Unauthorized to perform actions on a basket you do not own",
      })
...
export const getBasketWithTransactions = async (
  ...
  basket.currentbalance = await getCurrentBalance(Number(pid), String(name))
  basket.initialbalance = await getInitialBalance(Number(pid), String(name))
  return basket
}
